...
class MessageViewTestCase(TestCase):
    ...
    def setup_likes(self):
        ...
        m3 = Message(id= 9876, text='likable warble', user_id=self.testuser_id)
    ...
    def test_user_show_with_likes(self):
        ...
        with self.client as c:
            ...
            self.assertIn('@testuser', str(resp.data))
            soup = BeautifulSoup(str(resp.data), 'htl.parser')
            found = soup_find_all('li', {'class': 'stat'})
    ...
    def test_add_likes(self):
        ...
        with self.client as c:
            with c.session_transaction() as sess:
                sess[CURR_USER_KEY] = self.test_users_index

            resp = c.post('/messages/1984/like', follow_redirects=True)
            self.assertEqual(resp.status_code, 200)

            likes = Like.query.filter(Likes.message_id==1984).all()
    ...
    def test_show_followers(self):
        ...
        with self.client as c:
            ...
            resp = c.get(f'/users/{self.testuser_id}/follwers')
